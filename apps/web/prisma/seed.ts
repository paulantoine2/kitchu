...
type SeedSnapshot = {
  ...
  productReferences: Array<Prisma.ProductReferenceCreateManyInput & { stockQuantity?: number | null }>;
  ...
  cartItems: Array<{ recipeId: string; portions: number }>;
};
...
async function clearDatabase() {
  ...
  await prisma.legacyCartItem.deleteMany();
  await prisma.legacyProductStock.deleteMany();
...
async function seedFromSnapshot(snapshot: SeedSnapshot) {
  ...
  if (snapshot.productReferences.length > 0) {
    await prisma.productReference.createMany({
      data: snapshot.productReferences.map(({ stockQuantity, ...product }) => {
        void stockQuantity;
        return { ...product, ownerId: null };
      }),
    });
    const legacyStock = snapshot.productReferences.flatMap((product) =>
      product.id == null || product.stockQuantity == null
        ? []
        : [{ productReferenceId: product.id, stockQuantity: product.stockQuantity }],
    );
    if (legacyStock.length > 0) {
      await prisma.legacyProductStock.createMany({ data: legacyStock });
    }
  }
  ...
  if (snapshot.cartItems.length > 0) {
    await prisma.legacyCartItem.createMany({ data: snapshot.cartItems });
  }
}
