...
function UnitMobileItem({ unit, canEdit }: { unit: UnitRecord; canEdit: boolean }) {
  ...
  return (
    <Item
      ...
      render={isConfigurable && canEdit ? <Link href={`/units/${unit.id}`} /> : undefined}
    >
...
function UnitTableRow({ unit, canEdit }: { unit: UnitRecord; canEdit: boolean }) {
  const isConfigurable = !isHardcodedMeasurementKind(unit.kind);
        ...
        <>
          <ItemGroup className="stagger-children gap-2 md:hidden">
            {filteredUnits.map((unit) => (
              <UnitMobileItem key={unit.id} unit={unit} canEdit={canEdit} />
            ))}
          </ItemGroup>
          <div className="hidden animate-fade-up overflow-hidden rounded-2xl bg-card shadow-soft ring-1 ring-foreground/[0.05] md:block dark:ring-foreground/[0.08]">
            <Table>
              ...
              <TableBody>
                {filteredUnits.map((unit) => (
                  <UnitTableRow key={unit.id} unit={unit} canEdit={canEdit} />
                ))}
              </TableBody>
            </Table>
          </div>
