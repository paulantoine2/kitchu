...
import { AuthorizationError, requireAdmin } from "@/lib/auth-user";
...
export async function saveIngredient(payload: unknown) {
  ...
  try {
    ...
    const ingredient = await measurePerf("action:saveIngredient", "transaction", () =>
      ...
      const existingSharedProducts = await tx.productReference.findMany({
        where: { ingredientId: saved.id, ownerId: null },
        select: { id: true },
      });
      const existingSharedIds = new Set(existingSharedProducts.map((product) => product.id));
      ...
      for (const product of normalizedProducts) {
        ...
        const savedProduct = product.id && existingSharedIds.has(product.id)
          ? await tx.productReference.update({ where: { id: product.id }, data: productData })
          : await tx.productReference.create({ data: productData });
        savedProductIds.push(savedProduct.id);
      }
      ...
      await tx.productReference.deleteMany({
        where: {
          ingredientId: saved.id,
          ownerId: null,
          ...(savedProductIds.length > 0 ? { id: { notIn: savedProductIds } } : {}),
        },
      });
    ...
    const fullIngredient = await measurePerf("action:saveIngredient", "ingredient.findUniqueOrThrow", () =>
      ...
      include: {
        ...
        products: {
          ...
          include: {
            packageUnit: true,
            userStates: { where: { userId: admin.id }, select: { stockQuantity: true, priceOverride: true } },
          },
    ...
    const products = fullIngredient.products.map(({ userStates, ...product }) => ({
      ...product,
      catalogPrice: product.price,
      price: userStates[0]?.priceOverride ?? product.price,
      priceOverride: userStates[0]?.priceOverride ?? null,
      stockQuantity: userStates[0]?.stockQuantity ?? null,
    }));
  ...
  try {
    ...
    const ingredient = await prisma.ingredient.upsert({
      ...
      include: {
        baseUnit: true,
        units: { include: { unit: true }, orderBy: { unit: { name: "asc" } } },
        products: { where: { ownerId: null }, include: { packageUnit: true }, orderBy: { updatedAt: "desc" } },
      },
    });
...
export async function addIngredientUnitQuick(ingredientId: string, unitCode: string) {
  try {
    ...
    return {
      ...
      ingredient: {
        ...updatedIngredient,
        products: updatedIngredient.products.map((product) => ({
          ...product,
          catalogPrice: product.price,
          priceOverride: null,
          stockQuantity: null,
...
export async function deleteIngredient(id: string) {
  try {
    ...
    if (error instanceof AuthorizationError) return actionError(error);
    return {
      ok: false as const,
